...
indexes through `mirrors.kodi.tv`, which redirects each request to a randomly
chosen community mirror, so one bad mirror can stall it for minutes and then fail
it. Package imports never change the repository add-on, so a publish should not
wait on those mirrors; a change to the repository add-on or its tooling still has
to pass the checker before it deploys.
